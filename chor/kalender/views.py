...
def create_termin(request, calid, year=None, month=None, day=None):
    ...
    return render_to_response("kalender/new_termin.html", dict(calid=calid,form=form,menupoint="terminerstellen", ),context_instance=RequestContext(request))
...
def kalender_home_view(request):
    ...
    return render_to_response("kalender/home.html",{'day':today.day,'month':today.month,'month_name':month_names[today.month-1],'year':today.year,'Kalender':kalender,'next_7_days':next_7_days,'menupoint':"Home"},context_instance=RequestContext(request))
